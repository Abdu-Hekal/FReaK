function [F,P] = orig_KoopmanMilpRobust(phi,kList,kMax,ts,var,M)
% orig_KoopmanMilpRobust  constructs MILP constraints in YALMIP that compute
...
% :license: TBD
    ...
    switch (phi.type)
        ...
        case 'not'
            [Frest,Prest] = orig_KoopmanMilpRobust(phi.phi,kList,kMax,ts, var,M);
            [Fnot, Pnot] = not(Prest);
            F = [F, Fnot, Frest];
            P = Pnot;

        case 'or'
            [Fdis1,Pdis1] = orig_KoopmanMilpRobust(phi.phi1,kList,kMax,ts, var,M);
            [Fdis2,Pdis2] = orig_KoopmanMilpRobust(phi.phi2,kList,kMax,ts, var,M);
            [For, Por] = or([Pdis1;Pdis2],M);
            F = [F, For, Fdis1, Fdis2];
            P = Por;

        case 'and'
            [Fcon1,Pcon1] = orig_KoopmanMilpRobust(phi.phi1,kList,kMax,ts, var,M);
            [Fcon2,Pcon2] = orig_KoopmanMilpRobust(phi.phi2,kList,kMax,ts, var,M);
            [Fand, Pand] = and([Pcon1;Pcon2],M);
            F = [F, Fand, Fcon1, Fcon2];
            P = Pand;

        case '=>'
            [Fant,Pant] = orig_KoopmanMilpRobust(phi.phi1,kList,kMax,ts,var,M);
            [Fcons,Pcons] = orig_KoopmanMilpRobust(phi.phi2,kList,kMax,ts, var,M);
        ...
        case 'always'
            ...
            [Frest,Prest] = orig_KoopmanMilpRobust(phi.phi,kListAlw,kMax,ts, var,M);
        ...
        case 'eventually'
            ...
            [Frest,Prest] = orig_KoopmanMilpRobust(phi.phi,kListEv,kMax,ts, var,M);
        ...
        case 'until'
            [Fp,Pp] = orig_KoopmanMilpRobust(phi.phi1,kList,kMax,ts, var,M);
            [Fq,Pq] = orig_KoopmanMilpRobust(phi.phi2,kList,kMax,ts, var,M);
