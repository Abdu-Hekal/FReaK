...
% Check alpha limits
if(max(x_f16_hist(2,:)) > deg2rad(flightLimits.alphaMax) || ...
        min(x_f16_hist(2,:)) < deg2rad(flightLimits.alphaMin))
...
% Check beta limits
if(abs(x_f16_hist(3,:)) > deg2rad(flightLimits.betaMax))
    passFail.beta = false;
    passFail.stable = false;
else
    passFail.beta = true;
...
if(rad2deg(max_ps_accel) > flightLimits.psMaxAccel)
