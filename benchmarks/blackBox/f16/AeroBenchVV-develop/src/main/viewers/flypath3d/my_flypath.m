...
animateOn = false;          % animation: 'on','off'
axisOn = true;              % axes visibility: 'on','off'
...
plotGround = false;         % Determines if ground plane is displayed
fullTelemetry = false;      % Display telemetry on rendering
figName = 'Flypath';        % Set name of figure
...
while i <= length(varargin)
    switch lower(varargin{i})
        case 'animate'
            animateOn = strcmp(varargin{i+1},'on');
            i = i + 2;
        case 'axis'
            axisOn = strcmp(varargin{i+1},'on');
            i = i + 2;
        case 'axiscolor'
...
for i = 1:pStep:maxLength
    ...
    for j = 1:1:objCount
        ...
        transformMatrix = getTransformMatrix(theta,psi,phi);
...
function transformMatrix = getTransformMatrix(theta,psi,phi)
