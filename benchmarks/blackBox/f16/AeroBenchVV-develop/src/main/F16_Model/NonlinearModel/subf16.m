function xd=subf16(x,u)
%
%  xd = subf16(x,u)
%
%  This routine outputs the state vector derivative, xd, for the computer model of an F-16 aircraft.
%  This routine is called by the trimmer and jacob m-files.
%  Prior to running, this m-file must be edited to set the proper c.g. location (see 'xcg = ' below).
...
c1=-.770;c2=.02755;c3=1.055e-4;c4=1.642e-6;c5=.9604;c6=1.759e-2;c7=1.792e-5;c8=-.7336;c9=1.587e-5;
rtod=57.29578;g=32.17;

xd=x;
vt=x(1);alpha=x(2)*rtod;beta=x(3)*rtod;
phi=x(4);theta=x(5);psi=x(6);
p=x(7);q=x(8);r=x(9);alt=x(12);pow=x(13);
[amach,qbar]=adc(vt,alt);cpow=tgear(thtlc);
