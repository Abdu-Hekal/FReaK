...
%     "testCC5", globally(~globally(~implies(globally(x(2)-x(1)>=9,interval(0,5)),globally(x(5)-x(4)>=9,interval(5,20))),interval(0,8)),interval(0,72))
...
%     "testNNx", (~globally(x(1)<3.2,interval(0,1))) &
%     (~globally(~globally(x(1)>1.75 & x(1)<2.25,interval(0,0.5)),interval(1,1.5))) & (globally(x(1)>1.825 & x(1)<2.175,interval(2,3))) ; ... 
    };
benches{end+1} = bench;
...
bench.requirements = {; ...
    ...
    "AFC27", globally(implies(rise|fall,globally(abs(x(1))<beta,interval(1,5))),interval(11,50)); ... 
%     "AFC29",globally(abs(x(1))<gamma,interval(11,50)) ; ...
%     "AFC33", globally(abs(x(1))<gamma,interval(11,50)) ; ...
    };
benches{end+1} = bench;
